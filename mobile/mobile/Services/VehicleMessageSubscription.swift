...
class VehicleMessageService: NSObject, ObservableObject {
    ...
    var subscription: GraphQLSubscriptionOperation<VehicleMessage>?
    ...
    func startSubscription() {
        subscription = Amplify.API.subscribe(request: .onCreateVehicleMessage(owner: owner), valueListener: { (subscriptionEvent) in
